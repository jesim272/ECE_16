...
class Bt:
    ...
    def ble_setup(self):
        ...
        sleep(1)
        command = "AT+NAME?"
        self.ble_write(command)                          # Changed "Connected" to "AT+NAME?"
        sleep(1)
        response = ""                                    # new response
        while not ("PeripheralConnected" in response):       # read until PeripheralConnected to proceed
            self.ble_write(command)             #Send command until PeripheralConnected is received
            sleep(0.5)
            response = self.ble_read()
    ...
    def ble_read_line(self, eol=';'):                       #changed '\n' to ';'
        msg = ""
        c = ""
        while c != eol:
            msg += c
