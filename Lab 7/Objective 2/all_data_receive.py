...
peripheral_MAC = "D8A98BB5847E" #'D8A98BC2D4E4' #
serial_port = "COM3"
...
if write_flag:
    write_filename = "pedometer_training_active(lab8_2).csv"
...
def initialize_buffers():
    ...
    data_buffer[4] = ListBuffer([], maxlen=buffer_length)  # ir data filtered
...
def update_data():
    ...
    return [(data_buffer[0], data_buffer[2]), (data_buffer[0], data_buffer[4])]  # Plot t, ir data, filtered ir data
...
def cleanup():
    ...
    if read_flag and read_file:
        ...
        bt.ble_close()  # Try to close the BLE connection cleanly (may Sfail)
    print("Ran cleanup")
...
while True:
    try:
        ...
        ############################ added ########################################
        hr_calc = Hr(train_file = files[0], window_length = 100, plot=True)
        hr_calc1 = Hr(train_file = files[1], window_length = 100, plot=True)
